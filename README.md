...
Hologram Shader: For our first stylized visual effect we made a hologram shader made in a shader graph and it is applied to our dancing character model. This shader uses techniques like rim lighting, scanlines, transparency and color tinting. The goal for this shader is to bring a sci-fi kick into our game making the character glow, appear like a digital projection and glitch like a hologram. Just like how we used a Toon band to make toon shading we used a hologram band to make the special line effects on the model. Below is the formula for the hologram shader. For the dancing model we borrowed the animation from mixamo , hip hop dancing animation. 
